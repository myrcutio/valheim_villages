...
namespace ValheimVillages.NPCs.AI
{
    ...
    public static class HnaRegionGraph
    {
        ...
        {
            if (!s_initialized || s_regionIds.Count == 0) return "";

            var sb = new System.Text.StringBuilder();
            sb.Append(s_originX.ToString("F2", System.Globalization.CultureInfo.InvariantCulture));
            sb.Append(',');
            sb.Append(s_originZ.ToString("F2", System.Globalization.CultureInfo.InvariantCulture));
            sb.Append(',');
            sb.Append(CellSize.ToString("F2", System.Globalization.CultureInfo.InvariantCulture));
            ...
            foreach (string id in s_regionIds)
            {
                ...
                sb.Append(id);
                sb.Append(':');
                if (s_cellHeights.TryGetValue(id, out float h))
        ...
        /// </summary>
        public static bool Restore(string data)
        {
            ...
            if (header.Length >= 3)
            {
                ...
                {
                    if (Mathf.Abs(storedCellSize - CellSize) > 0.01f)
                    {
                        Plugin.Log?.LogInfo(
                            $"[HNA] ZDO graph cell size mismatch ({storedCellSize}m vs current {CellSize}m), " +
            ...
            {
                // Old format without cell size — reject, force regeneration
            ...
            SetGraph(ox, oz, regionIds, new List<HnaLink>(), cellHeights);
