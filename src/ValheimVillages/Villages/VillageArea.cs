...
namespace ValheimVillages.Villages
{
    ...
    public class VillageArea
    {
        ...
        public bool IsNearBoundary(Vector3 position, float radius)
        {
            ...
            for (var i = 0; i < m_polygon2D.Count; i++)
            {
                var next = (i + 1) % m_polygon2D.Count;
                var distSq = PointToSegmentDistanceSq(point, m_polygon2D[i], m_polygon2D[next]);
                if (distSq <= radiusSq)
                    return true;
            }
