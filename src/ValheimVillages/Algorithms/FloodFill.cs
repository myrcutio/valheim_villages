    ...
    public static class FloodFill
    {
            ...
            float cellY, List<Barrier> barriers)
        {
            for (var i = 0; i < barriers.Count; i++)
            {
                ...
                var dA = (ax - b.Px) * b.Fx + (az - b.Pz) * b.Fz;
                var dB = (bx - b.Px) * b.Fx + (bz - b.Pz) * b.Fz;
                if (dA * dB > 0) continue;
                var denom = dA - dB;
                ...
                var cx = ax + t * (bx - ax);
                var cz = az + t * (bz - az);
                var dist2 = (cx - b.Px) * (cx - b.Px) + (cz - b.Pz) * (cz - b.Pz);
            ...
            int cellCountX, int cellCountZ)
        {
            ...
            var r2 = FloodFillRadius * FloodFillRadius;

            // Seed from beds
            foreach (var bed in beds)
            {
                var ix = (int)Math.Floor((bed.x - originX) / cellSize);
                var iz = (int)Math.Floor((bed.z - originZ) / cellSize);
            ...
            while (queue.Count > 0)
            {
                ...
                for (var dz = -1; dz <= 1; dz++)
                {
                    ...
                    var wx = originX + (nx + 0.5f) * cellSize;
                    var wz = originZ + (nz + 0.5f) * cellSize;
                    ...
                    // Slope check
                    var dxzLen = (float)Math.Sqrt(
                        (wx - posA.Wx) * (wx - posA.Wx) + (wz - posA.Wz) * (wz - posA.Wz));
                    if (dxzLen < 0.01f) continue;
                    var dy = Math.Abs(hitY - posA.Y);
                    var slopeDeg = (float)Math.Atan2(dy, dxzLen) * (180f / (float)Math.PI);
