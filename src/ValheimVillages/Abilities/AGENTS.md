...
Keywords: ability, passive effect, Mountain Stride, spawn block, spawn protection, status effect, SE_MountainStride,
VillagerAbilityManager, IAbility, IPlayerAbility, IPassiveEffect, RegisterAbility, RegisterPassive, sliding immunity,
cooldown, village safety

## Purpose

Player abilities and passive village effects. Abilities are teachable skills (e.g., Mountain Stride) activated by the
player. Passive effects (e.g., spawn block) apply automatically based on village area.
...
- `[RegisterAbility("mountainstride")]` and `[RegisterPassive("spawnblock")]` discovered by
...
- **Villager/** -- Villager type definitions (Villager/Registry/Definitions/*.json) list passive/ability tags in
  `benefits` arrays.
