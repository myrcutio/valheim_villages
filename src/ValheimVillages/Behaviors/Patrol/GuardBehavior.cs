...
namespace ValheimVillages.Behaviors.Patrol
{
    ...
    public class GuardBehavior
    {
        ...
        private readonly GuardPatrolRefiner m_refiner = new();
        private GuardPatrolDiscovery m_discovery;
        ...
        public GuardBehavior(VillagerAI ai)
        {
            m_ai = ai;
            m_lastProgressPosition = ai.Position;
        }
        ...
        public Vector3 BedPosition => m_ai.Memory.BedPosition;
        ...
        /// <summary>
        /// Reset the guard's village mapping. Clears waypoints, unregisters the village
        /// area, and transitions back to Idle so discovery restarts on the next tick.
        /// </summary>
        public void ResetDiscovery()
        {
            Plugin.Log?.LogWarning($"[Guard:{m_ai.NpcName}] Discovery reset requested (debug)");
        ...
        public void RestoreState(List<VillagerWaypoint> waypoints, int wpIndex, Vector3? breach, bool isHna = false)
        {
            ...
            RegisterVillageArea();
            Plugin.Log?.LogInfo(
                $"[Guard:{m_ai.NpcName}] Restored {m_patrolWaypoints.Count} waypoints" +
                $" (hna={isHna}), index={m_currentWaypointIndex}");
        }

        /// <summary>
        /// Returns the desired behavior tick interval for the guard's current state.
        ...
        /// <summary>Main guard AI update, called by VillagerAI each behavior tick.</summary>
        public void UpdateGuardAI(float dt)
        {
        ...
        /// <summary>Called by VillagerAI when the guard arrives at its target.</summary>
        public void HandleArrival()
        {
            ...
            switch (m_ai.CurrentState)
            {
                ...
                case BehaviorState.Traveling:
                    // Could be breach walk or debug command -- go Idle and let UpdateGuardAI decide next
                    m_ai.SetState(BehaviorState.Idle);
                    break;
                case BehaviorState.Sleeping:
                    m_ai.Instance.StopMoving();
                    m_ai.EnterSleepAnimation();
        ...
        /// <summary>Send the guard to the breach location ("Show me" button).</summary>
        ...
        private void HandlePatrolArrival()
        {
            ...
            if (!m_isHnaRoute)
            {
                ...
                if (m_refiner.IsQuickArrival() && m_patrolWaypoints != null && ActiveWaypointCount > 3
                    && !GuardPatrolRefiner.WouldCreateLargeGap(
                        m_patrolWaypoints, m_currentWaypointIndex, m_ai.Memory.BedPosition))
                {
                    Plugin.Log?.LogInfo(
                        $"[Guard:{m_ai.NpcName}] Quick arrival at waypoint {m_currentWaypointIndex}, deactivating redundant point");
                ...
                if (m_patrolWaypoints != null && m_currentWaypointIndex < m_patrolWaypoints.Count)
                {
                    ...
                    if (GuardPatrolRefiner.TryNudgeTowardWall(
                            currentWp.Position, m_ai.Memory.BedPosition, out var nudged))
                    {
                        m_patrolWaypoints[m_currentWaypointIndex] =
                            new VillagerWaypoint(nudged, PathingStrategyRegistry.GuardPatrolId);
                        Plugin.Log?.LogInfo(
                            $"[Guard:{m_ai.NpcName}] Nudged waypoint {m_currentWaypointIndex} toward wall" +
        ...
        /// Sets the guard to Alarmed state if a breach was detected.
        /// </summary>
        private void OnBreachCheckResult(TaskResult result)
        {
                ...
                float.TryParse(bz, NumberStyles.Float, CultureInfo.InvariantCulture, out float z))
            {
                ...
                Plugin.Log?.LogWarning(
                    $"[Guard:{m_ai.NpcName}] BREACH detected (async) at ({x:F0},{z:F0})!");
            }
        }

        private void StartDiscovery()
        {
            // If HNA graph is not in memory, try restoring from this guard's ZDO
            if (!HnaRegionGraph.IsAvailable)
            {
                var zdo = m_ai.NView?.GetZDO();
                if (zdo != null && GuardPersistence.TryRestoreHnaGraph(zdo))
                    Plugin.Log?.LogInfo($"[Guard:{m_ai.NpcName}] Restored HNA graph from ZDO");
            }
            ...
            if (hnaWaypoints.Count >= 3)
            {
                m_patrolWaypoints = hnaWaypoints
                    .Select(p => new VillagerWaypoint(p, PathingStrategyRegistry.GuardPatrolId))
                    .ToList();
                m_isHnaRoute = true;
                m_hnaPartitionRequested = false;
            ...
            if (!m_hnaPartitionRequested)
            {
                ...
                var bedPos = m_ai.Memory.BedPosition;
                Plugin.Log?.LogInfo($"[Guard:{m_ai.NpcName}] HNA graph unavailable, requesting hna_partition build");
                GlobalTaskQueue.Enqueue(new VillagerTask
                {
                    Name = "hna_partition",
                    SourceId = $"guard:{m_ai.UniqueId}",
            ...
            Plugin.Log?.LogInfo($"[Guard:{m_ai.NpcName}] HNA partition completed but graph insufficient, falling back to raycast discovery");
            m_isHnaRoute = false;
            m_hnaPartitionRequested = false;
            m_discovery = new GuardPatrolDiscovery(m_ai, m_ai.Memory.BedPosition);
        ...
        private void CompleteHnaDiscovery()
        {
            ...
            var zdo = m_ai.NView?.GetZDO();
            if (zdo != null)
                GuardPersistence.SaveHnaGraph(zdo);

            Plugin.Log?.LogInfo(
                $"[Guard:{m_ai.NpcName}] HNA boundary discovery complete, {m_patrolWaypoints.Count} waypoints");
            AdvanceToNextWaypoint();
        }
        ...
        private void CompleteDiscovery()
        {
            ...
            RegisterVillageArea();
            SaveState();
            Plugin.Log?.LogInfo($"[Guard:{m_ai.NpcName}] Discovery complete, {m_patrolWaypoints.Count} waypoints");
            AdvanceToNextWaypoint();
        }
        ...
        {
            var zdo = m_ai.NView?.GetZDO();
            if (zdo != null) GuardPersistence.Save(this, zdo);
        }
        ...
        private void AdvanceToNextWaypoint()
        {
            ...
            if (wrappedToZero && !m_isHnaRoute && ActiveWaypointCount >= 3)
            {
                ...
                // 1. Sort clockwise so the guard walks a consistent loop
                GuardPatrolRefiner.SortClockwise(m_patrolWaypoints, bed);

                // 2. Fill angular gaps to ensure full perimeter coverage
                int filled = GuardPatrolRefiner.FillAngularGaps(m_patrolWaypoints, bed);
                if (filled > 0)
                {
                    Plugin.Log?.LogInfo(
                        $"[Guard:{m_ai.NpcName}] Filled {filled} angular gaps ({m_patrolWaypoints.Count} total)");
                    changed = true;
                }

                // 3. Verify the patrol polygon encloses the bed
                if (!GuardPatrolRefiner.PolygonContainsBed(m_patrolWaypoints, bed))
                    Plugin.Log?.LogWarning($"[Guard:{m_ai.NpcName}] Patrol polygon does not enclose bed");

                // 4. Prune redundant collinear waypoints (with arc-length veto)
                if (m_patrolWaypoints.Count >= 4)
                {
                    int removed = GuardPatrolRefiner.OptimizeCircuit(m_patrolWaypoints, bed);
                    if (removed > 0)
                    {
                        Plugin.Log?.LogInfo(
                            $"[Guard:{m_ai.NpcName}] Optimized: removed {removed} redundant waypoints" +
            ...
            Plugin.Log?.LogInfo(
                $"[Guard:{m_ai.NpcName}] Patrol -> waypoint {m_currentWaypointIndex}/{m_patrolWaypoints.Count}" +
        ...
        /// <summary>
        /// Check if the guard is making progress toward its movement target.
        ...
        /// Patrolling: uses the refiner to insert intermediate waypoints (guard is making
        /// progress but taking long) or relocate unreachable waypoints (guard is stuck).
        /// </summary>
        private void CheckStallAndRecover(BehaviorState state)
        {
            ...
            if (state == BehaviorState.CircuitTracing)
            {
                ...
                {
                    ResetStallTracking();
                    Plugin.Log?.LogWarning($"[Guard:{m_ai.NpcName}] Stuck during circuit tracing (~5s no movement), skipping arc point");
            ...
            if (m_isHnaRoute)
            {
                ...
                {
                    Plugin.Log?.LogWarning(
                        $"[Guard:{m_ai.NpcName}] Stuck on HNA waypoint {m_currentWaypointIndex}, " +
        ...
        /// one in route order (so the guard advances forward, not backward into a loop).
        ...
        private void RecoverToNearestReachableWaypoint()
        {
            ...
            if (!NavMesh.SamplePosition(guardPos, out NavMeshHit srcHit, 10f, filter))
            {
                Plugin.Log?.LogWarning(
                    $"[Guard:{m_ai.NpcName}] Recovery failed: guard position not on NavMesh, skipping");
            ...
            {
                Plugin.Log?.LogInfo(
                    $"[Guard:{m_ai.NpcName}] Recovery: skipping stuck wp {stuckIdx}, " +
            ...
            {
                Plugin.Log?.LogWarning(
                    $"[Guard:{m_ai.NpcName}] Recovery failed: no reachable waypoint found, " +
        ...
        private void DeactivateCurrentWaypoint()
        {
            ...
            wp.Active = false;
            Plugin.Log?.LogWarning(
                $"[Guard:{m_ai.NpcName}] Deactivated waypoint {m_currentWaypointIndex}" +
                $" ({ActiveWaypointCount} active / {m_patrolWaypoints.Count} total)");

            if (ActiveWaypointCount == 0)
            {
                Plugin.Log?.LogWarning($"[Guard:{m_ai.NpcName}] No active waypoints left, restarting discovery");
                m_discoveryComplete = false;
                VillageAreaManager.UnregisterArea(m_ai.UniqueId);
                SaveState();
                m_ai.SetState(BehaviorState.Idle);
        ...
        /// <summary>
        /// Insert a new waypoint at the guard's current position, before the current target.
        /// The guard continues moving toward the original target; the new point is for future laps.
        /// </summary>
        private void InsertWaypointAtCurrentPosition()
        {
            if (m_patrolWaypoints == null) return;

            var wp = new VillagerWaypoint(m_ai.Position, PathingStrategyRegistry.GuardPatrolId);
            m_patrolWaypoints.Insert(m_currentWaypointIndex, wp);
            m_currentWaypointIndex++;
            RegisterVillageArea();
            SaveState();
            Plugin.Log?.LogInfo(
                $"[Guard:{m_ai.NpcName}] Inserted mid-traverse waypoint at ({m_ai.Position.x:F0},{m_ai.Position.z:F0})" +
                $" ({m_patrolWaypoints.Count} total)");
        }

        /// <summary>
        /// Deactivate the unreachable target waypoint and insert a new active waypoint
        /// at the guard's current position. The old waypoint is retained for potential reactivation.
        /// </summary>
        private void RelocateCurrentWaypoint()
        {
            ...
            var newWp = new VillagerWaypoint(m_ai.Position, PathingStrategyRegistry.GuardPatrolId);
            m_patrolWaypoints.Insert(m_currentWaypointIndex + 1, newWp);

            Plugin.Log?.LogWarning(
                $"[Guard:{m_ai.NpcName}] Deactivated unreachable waypoint {m_currentWaypointIndex}," +
