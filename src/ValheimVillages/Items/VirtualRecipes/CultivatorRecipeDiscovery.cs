...
namespace ValheimVillages.Items.VirtualRecipes
{
    ...
    public static class CultivatorRecipeDiscovery
    {
        ...
        public static bool MatchesExclusion(string text, IReadOnlyList<string> exclusionSubstringsLower)
        {
            ...
            for (var i = 0; i < exclusionSubstringsLower.Count; i++)
            {
        ...
        /// </summary>
        public static List<VirtualRecipeEntry> GetPlantingRecipes(HashSet<string> existingOutputs,
            IReadOnlyList<string> exclusionSubstringsLower)
        {
            ...
            var pieceGos = GetPieceGameObjects(buildPieces, out var fromMCount, out var fromAvailableCount);
            if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                Plugin.Log.LogInfo(
                    $"[Valheim Villages] Cultivator discovery: m_pieces count={fromMCount}, m_availablePieces total pieces={fromAvailableCount}, merged unique={pieceGos?.Count ?? 0}");
            ...
            foreach (var pieceGo in pieceGos)
            {
                ...
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug($"[Valheim Villages] Piece skip (no Piece component): {pieceGo.name}");
                    continue;
                }
                ...
                if (MatchesExclusion(pieceName, exclusionSubstringsLower))
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug(
                            $"[Valheim Villages] Piece filtered out (piece name): name='{pieceName}' (gameObject='{pieceGo.name}')");
                    continue;
                }

                var outputItemName = ResolveOutputItemName(pieceGo, piece);
                if (string.IsNullOrEmpty(outputItemName))
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug(
                            $"[Valheim Villages] Piece no output: name='{pieceName}' gameObject='{pieceGo.name}'");
                    continue;
                }
                ...
                if (MatchesExclusion(outputItemName, exclusionSubstringsLower))
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug(
                            $"[Valheim Villages] Piece filtered out (output): output={outputItemName} name='{pieceName}'");
                    continue;
                }
                ...
                if (existingOutputs != null && existingOutputs.Contains(outputItemName))
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug(
                            $"[Valheim Villages] Piece skipped (already in JSON): output={outputItemName} name='{pieceName}'");
                    continue;
                }
                ...
                if (inputs == null || inputs.Length == 0)
                {
                    if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                        Plugin.Log.LogDebug(
                            $"[Valheim Villages] Piece no inputs: name='{pieceName}' output={outputItemName}");
                    continue;
                }

                var inputExcluded = false;
                for (var ii = 0; ii < inputs.Length; ii++)
                    if (MatchesExclusion(inputs[ii].item, exclusionSubstringsLower))
                    {
                        ...
                        if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                            Plugin.Log.LogDebug(
                                $"[Valheim Villages] Piece filtered out (ingredient): input={inputs[ii].item} name='{pieceName}'");
                        break;
                    }
                ...
                if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                    Plugin.Log.LogInfo(
                        $"[Valheim Villages] Cultivator recipe added: '{pieceName}' -> {outputItemName} (inputs: {string.Join(", ", Array.ConvertAll(inputs, i => $"{i.item}x{i.amount}"))})");
            ...
            if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
                Plugin.Log.LogInfo($"[Valheim Villages] Cultivator discovery total recipes added: {list.Count}");
            return list;
        }

        /// <summary>
        ///     Get all piece GameObjects from the table; merge m_pieces and m_availablePieces so mod-added plantable types
        ///     are included.
        /// </summary>
        private static List<GameObject> GetPieceGameObjects(PieceTable table, out int fromMPiecesCount,
            out int fromAvailablePiecesCount)
        {
            fromMPiecesCount = 0;
            ...
            if (CultivatorDiscoveryDebug.Enabled && Plugin.Log != null)
            {
                ...
                DebugLog.List("Cultivator", "piece_list", names);
            }
        ...
        private static string ResolveOutputItemName(GameObject pieceGo, Piece piece)
        {
            ...
            if (plant != null)
            {
                ...
                if (grownPrefabs != null && grownPrefabs.Length > 0)
                    for (var i = 0; i < grownPrefabs.Length; i++)
                    {
