...
public static class PathScoring
{
    ...
    /// </summary>
    /// <param name="pipeline">Waypoints produced by the boundary pipeline.</param>
    /// <param name="reference">Player-walked reference positions.</param>
    /// <param name="coverageRadius">Max distance for a reference point to be "covered".</param>
        ...
        List<Vec3> pipeline, List<Vec3> reference, float coverageRadius = 3f)
    {
        ...
        // Hausdorff: max(max_ref(min_pipe), max_pipe(min_ref))
        ...
        float hausdorff = MathF.Max(maxRefToPipe, maxPipeToRef);
    ...
    /// Instead of nearest-point, find the nearest point on any segment of the pipeline polygon.
    /// More accurate for sparse waypoint sets that define a polygon.
    /// </summary>
    ...
    /// <summary>
    /// Find the nearest XZ distance from a point to any segment of the closed polygon.
    /// </summary>
    ...
    private static float PointToSegmentDistXZ(Vec3 p, Vec3 a, Vec3 b)
    {
        ...
        float t = Math.Clamp(
            ((p.X - a.X) * abx + (p.Z - a.Z) * abz) / lenSq, 0f, 1f);
